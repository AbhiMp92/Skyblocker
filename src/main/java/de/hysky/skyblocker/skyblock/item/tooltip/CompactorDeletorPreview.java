...
public class CompactorDeletorPreview {
    ...
    public static boolean drawPreview(DrawPreviewParameters params) {
        int targetIndex = getTargetIndex(params.getTooltips());
        if (targetIndex == -1) return false;

        // Get items in compactor or deletor
        NbtCompound customData = ItemUtils.getCustomData(params.getStack());
        ...
        List<IntObjectPair<ItemStack>> slots = customData.getKeys().stream()
                .filter(slot -> slot.contains(params.getType().toLowerCase().substring(0, 7)))
                .map(slot -> IntObjectPair.of(Integer.parseInt(slot.substring(17)), ItemRepository.getItemStack(customData.getString(slot))))
                .toList();

        List<TooltipComponent> components = params.getTooltips().stream().map(Text::asOrderedText).map(TooltipComponent::of).collect(Collectors.toList());
        IntIntPair dimensions = DIMENSIONS.getOrDefault(params.getSize(), DEFAULT_DIMENSION);
        ...
        if (slots.isEmpty()) {
            ...
            ((DrawContextInvoker) params.getContext()).invokeDrawTooltip(client.textRenderer, components, params.getX(), params.getY(), HoveredTooltipPositioner.INSTANCE);
            return true;
        }
