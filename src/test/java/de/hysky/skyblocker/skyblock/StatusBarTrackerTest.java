...
class StatusBarTrackerTest {
    ...
    void normalStatusBar() {
        ...
        assertNull(res);
        StatusBarAssert.assertStats(tracker, 934, 1086, 159, 562, 516, 0);
    }
    ...
    void overflowMana() {
        ...
        assertNull(res);
        StatusBarAssert.assertStats(tracker, 1605, 1305, 270, 548, 548, 200);
    }
...
class StatusBarAssert {

    public static void assertStats(StatusBarTracker tracker, int hp, int maxHp, int def, int mana, int maxMana, int overflowMana) {
        int absorption = 0;
        if (hp > maxHp) {
